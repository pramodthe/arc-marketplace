...
from .agent import HotelFinderAgent
...
async def invoke(body: InvokeBody) -> dict[str, Any]:
    ...
    try:
        ...
        recommendation = f"Hotel recommendation failed: {exc}"

    return {
        "agent": "hotel_finder_agent",
        "promptReceived": prompt,
        "result": {"recommendation": recommendation},
        "summary": "Hotel options generated from internal mock simulation data.",
    }
...
async def a2a_message(
    ...
    return handle_a2a_jsonrpc(
        body,
        run=agent.run,
        agent_label="Hotel recommendation",
    )
